...
Utter is designed so your speech never leaves your Mac. This document states
...
1. On first launch Utter downloads the speech-recognition model (~626 MB) from
...
After that Utter works fully offline. You can verify this with Little Snitch,
...
| What | Where | Default |
|---|---|---|
| Dictation history (plain text) | `~/Library/Application Support/Utter/dictation-history.md` | Kept — turn off with `keepDictationHistory` |
| Meeting transcripts (Markdown) | `~/Documents/Utter/Meetings/` | Kept |
| Raw meeting audio (WAV) | `~/Documents/Utter/Recordings/` | **Deleted** after transcription — opt in via *Keep Meeting Audio Files* |
| Models | `~/Library/Application Support/Utter/Models/` | Kept |
| Settings | `defaults` domain `com.jancuhel.utter` | — |
...
- **`~/Documents` is synced to iCloud Drive** for anyone who enabled "Desktop &
  ...
  leave your Mac — not to Utter, but to Apple. Turn that off, or move the files.
...
To delete everything Utter ever wrote:

```bash
rm -rf ~/Library/Application\ Support/Utter ~/Documents/Utter
...
Utter shows a one-time warning before your first meeting recording, but the
legal responsibility is yours, not the software's.

If you are in the EU and record other people, **you** are the data controller
for those recordings. The author of Utter is not — no data ever reaches them.

## Permissions Utter asks for
...
- **Accessibility** — to type the transcript where your cursor is. This is a
  ...
  (`Sources/UtterCore/TextInserter.swift`).
- **System Audio Recording** — only for meeting mode, to hear other participants.

Utter is not sandboxed, because the Core Audio process tap and caret insertion
