...
Utter is licensed under the Apache License 2.0 (see `LICENSE`). It builds on the
following third-party components. Each remains under its own license and
copyright.

## Statically linked into the Utter binary
...
## Downloaded at runtime (not distributed with Utter)
...
Downloaded on first launch into `~/Library/Application Support/Utter/Models`.
The underlying [Whisper](https://github.com/openai/whisper) model and code are
© OpenAI, MIT licensed. Utter deliberately does **not** bundle these weights;
they are fetched from their original source at runtime.

## Optional, installed by the user (not distributed with Utter)

**llama.cpp** — © 2023 The ggml authors, MIT licensed. Installed by the user via
Homebrew (`brew install llama.cpp`). Utter launches `llama-server` as a separate
...
into `~/Library/Application Support/Utter/Models`. Not redistributed by Utter.

## Apple frameworks

Utter uses AppKit, AVFoundation, CoreAudio, CoreML, Carbon (HIToolbox),
