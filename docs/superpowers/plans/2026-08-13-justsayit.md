...
>
> **Deviation note:** authored and executed inline in the same autonomous session (`/goal` mode, no user available to choose execution mode). Tasks below carry exact files/interfaces/commands; full code lives in the commits, spec in `docs/superpowers/specs/2026-08-13-justsayit-design.md`.
...
**Architecture:** SwiftPM executable bundled as `JustSayIt.app` (LSUIElement). WhisperKit (CoreML/ANE) for ASR, Carbon hotkeys, AVAudioEngine mic capture, Core Audio process tap for system audio, `llama-server` subprocess (Qwen3-4B GGUF) for cleanup.
...
- All model/data paths under `~/Library/Application Support/JustSayIt/`; meeting outputs under `~/Documents/JustSayIt/Meetings/`.
...
**Files:** Create `Package.swift`, `Sources/JustSayIt/main.swift` (temporary stub), `Sources/JustSayIt/SettingsStore.swift`, `Tests/JustSayItTests/SettingsStoreTests.swift`, `.gitignore`.
...
**Files:** Create `Sources/JustSayIt/TranscriptMerger.swift`, `Tests/JustSayItTests/TranscriptMergerTests.swift`.
...
**Files:** Create `Sources/JustSayIt/MicRecorder.swift`, `Sources/JustSayIt/WavFileWriter.swift`, `Tests/JustSayItTests/WavFileWriterTests.swift`.
...
**Files:** Create `Sources/JustSayIt/Transcriber.swift`.
...
**Files:** Create `Sources/JustSayIt/HotkeyManager.swift`, `Sources/JustSayIt/HUD.swift`, `Sources/JustSayIt/StatusItemController.swift`, `Sources/JustSayIt/DictationController.swift`, `Sources/JustSayIt/AppDelegate.swift`; replace `main.swift` stub with NSApplication bootstrap.
...
**Files:** Create `Sources/JustSayIt/TextInserter.swift`, `Sources/JustSayIt/Permissions.swift`.
...
**Files:** Create `Sources/JustSayIt/SystemAudioTap.swift`, `Sources/JustSayIt/MeetingController.swift`.
...
**Files:** Create `Sources/JustSayIt/LLMCleaner.swift`, `Sources/JustSayIt/LLMPrompts.swift`, `Tests/JustSayItTests/LLMPromptsTests.swift`.
...
- [ ] `build_app.sh`: `swift build -c release` → assemble `build/JustSayIt.app` (Info.plist: CFBundleIdentifier `com.jancuhel.justsayit`, LSUIElement, NSMicrophoneUsageDescription, NSAudioCaptureUsageDescription, LSMinimumSystemVersion 14.4) → `codesign --force -s -` → optional `--install` to `~/Applications`.
- [ ] `setup_llm.sh`: brew install llama.cpp if missing; curl Qwen3-4B-Instruct-2507-Q4_K_M.gguf (unsloth GGUF repo) to Models dir with resume; write resolved paths into UserDefaults via `defaults write com.jancuhel.justsayit`.
