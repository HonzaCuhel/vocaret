# Vocaret Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
>
> **Deviation note:** authored and executed inline in the same autonomous session (`/goal` mode, no user available to choose execution mode). Tasks below carry exact files/interfaces/commands; full code lives in the commits, spec in `docs/superpowers/specs/2026-08-13-vocaret-design.md`.
...
**Architecture:** SwiftPM executable bundled as `Vocaret.app` (LSUIElement). WhisperKit (CoreML/ANE) for ASR, Carbon hotkeys, AVAudioEngine mic capture, Core Audio process tap for system audio, `llama-server` subprocess (Qwen3-4B GGUF) for cleanup.
...
- All model/data paths under `~/Library/Application Support/Vocaret/`; meeting outputs under `~/Documents/Vocaret/Meetings/`.
...
**Files:** Create `Package.swift`, `Sources/Vocaret/main.swift` (temporary stub), `Sources/Vocaret/SettingsStore.swift`, `Tests/VocaretTests/SettingsStoreTests.swift`, `.gitignore`.
...
**Files:** Create `Sources/Vocaret/TranscriptMerger.swift`, `Tests/VocaretTests/TranscriptMergerTests.swift`.
...
**Files:** Create `Sources/Vocaret/MicRecorder.swift`, `Sources/Vocaret/WavFileWriter.swift`, `Tests/VocaretTests/WavFileWriterTests.swift`.
...
**Files:** Create `Sources/Vocaret/Transcriber.swift`.
...
**Files:** Create `Sources/Vocaret/HotkeyManager.swift`, `Sources/Vocaret/HUD.swift`, `Sources/Vocaret/StatusItemController.swift`, `Sources/Vocaret/DictationController.swift`, `Sources/Vocaret/AppDelegate.swift`; replace `main.swift` stub with NSApplication bootstrap.
...
**Files:** Create `Sources/Vocaret/TextInserter.swift`, `Sources/Vocaret/Permissions.swift`.
...
**Files:** Create `Sources/Vocaret/SystemAudioTap.swift`, `Sources/Vocaret/MeetingController.swift`.
...
**Files:** Create `Sources/Vocaret/LLMCleaner.swift`, `Sources/Vocaret/LLMPrompts.swift`, `Tests/VocaretTests/LLMPromptsTests.swift`.
...
- [ ] `build_app.sh`: `swift build -c release` → assemble `build/Vocaret.app` (Info.plist: CFBundleIdentifier `com.jancuhel.vocaret`, LSUIElement, NSMicrophoneUsageDescription, NSAudioCaptureUsageDescription, LSMinimumSystemVersion 14.4) → `codesign --force -s -` → optional `--install` to `~/Applications`.
- [ ] `setup_llm.sh`: brew install llama.cpp if missing; curl Qwen3-4B-Instruct-2507-Q4_K_M.gguf (unsloth GGUF repo) to Models dir with resume; write resolved paths into UserDefaults via `defaults write com.jancuhel.vocaret`.
